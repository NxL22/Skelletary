...
function getCategoryBadgeTone(category = "") {
  ...
  // Tonos por modalidad. Importante: ninguno puede ser rose/lavender/violet
  // porque esos colores estan reservados para el chip de "Variable a completar
  // antes de copiar" y queremos que el ojo los distinga sin esfuerzo.
  // Paleta usada:
  //   - angio      -> sky-400 (celeste)
  //   - doppler    -> cyan (turquesa)
  //   - ecografia  -> emerald-400 (verde)
  //   - resonancia -> fuchsia-400 (magenta)
  //   - rx         -> amber-300 (amarillo)
  //   - tac        -> teal-400 (verde azulado)
  //   - mamografia -> pink-400 (rosa fuerte, distinto del rose de variables)
  if (normalizedCategory.includes("angio")) {
    return "border border-sky-400/30 bg-sky-400/10 text-sky-300";
  }

  if (normalizedCategory.includes("doppler")) {
    return "border border-cyan/30 bg-cyan/10 text-cyan";
  }
  ...
  if (normalizedCategory.includes("resonancia") || /\brm\b/.test(normalizedCategory)) {
    return "border border-fuchsia-400/30 bg-fuchsia-400/10 text-fuchsia-300";
  }
  ...
  if (
    ...
    return "border border-amber-300/30 bg-amber-300/10 text-amber-300";
  }
  ...
  if (normalizedCategory.includes("mamograf")) {
    return "border border-pink-400/30 bg-pink-400/10 text-pink-300";
  }
...
<span className="badge-soft font-mono text-cyan" title={shortcutHelpText}>
            ...
            <div className="flex min-h-[1.75rem] min-w-0 items-start">
              {variableEnabled ? (
<span className="badge-soft text-lavender">
...
<div className={`mt-4 flex-1 rounded-[22px] bg-slate-950/40 p-4 ${CARD_PREVIEW_MIN_HEIGHT_CLASS}`}>
        {variableEnabled ? (
          <div className="mb-4 rounded-2xl border border-lavender/20 bg-lavender/10 p-3">
